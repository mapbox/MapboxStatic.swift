...
class MapboxStaticTests: XCTestCase {
    ...
    func testOverlayGeoJSON() {
        ...
        stub(condition: isHost(geojsonURL.host!)) { request in
            return fixture(Bundle(for: type(of: self)).path(forResource: "polyline", ofType: "geojson")!,
                headers: nil)
        }

        let geojsonString = try! NSString(contentsOf: geojsonURL, encoding: String.Encoding.utf8.rawValue)
...
//        let path = Path(coordinates: [])
    ...
    func testAutoFit() {
        ...
        let markerOverlay = Marker(
            ...
            size: .medium,
            iconName: "cafe")
        markerOverlay.color = Color.brown
