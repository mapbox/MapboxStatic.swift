...
/*:
 ...
 MapboxStatic.swift makes it easy to connect your iOS Cocoa Touch application to the [Mapbox Static API](https://www.mapbox.com/api-documentation/maps/#static) or the [classic Mapbox Static API](https://www.mapbox.com/api-documentation/legacy/static-classic). Quickly generate a map snapshot – a static map image with overlays – by fetching it synchronously or asynchronously over the Web using first-class Swift or Objective-C data types.
 
 A snapshot is a flattened PNG or JPEG image, ideal for use in a table or image view, user notification, or anyplace else you’d like a quick, custom map without the overhead of an interactive view. A static map is created in a single HTTP request. Overlays are added server-side.
 ...
 To generate a snapshot from a style, you’ll need its [style URL](https://www.mapbox.com/help/define-style-url/). You can either choose a [Mapbox-designed style](https://www.mapbox.com/api-documentation/maps/#styles) or design one yourself in [Mapbox Studio](https://www.mapbox.com/studio/styles/). You can use the same style in the Mapbox Maps SDK for iOS.
 
 To generate a snapshot from a tile set, you’ll need a [map ID](https://www.mapbox.com/help/define-map-id/). You can either choose a [Mapbox-maintained tile set](https://www.mapbox.com/api-documentation/maps/#maps) or upload your own to [Mapbox Studio](https://www.mapbox.com/studio/tilesets/).
...
/*:
 ...
 When creating a classic snapshot, you can also specify PNG or JPEG image format as well as various [bandwidth-saving image qualities](https://www.mapbox.com/api-documentation/legacy/static-classic/#retrieve-a-static-map-image).
